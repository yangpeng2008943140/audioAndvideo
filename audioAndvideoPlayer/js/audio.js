$(function() {
	...
	var arrmusics = [
		...
		["http://localhost:8090/music/绅士 - 薛之谦.mp3", "绅士"],
		["http://localhost:8090/music/一半 - 薛之谦.mp3", "一半"],
		["http://localhost:8090/music/认真的雪 - 薛之谦.mp3", "认真的雪"]
	];
	var index = 3; //歌曲索引
	var Totaltime //歌曲时间
	var mute = false; //歌曲静音
	...
	$(".start").click(function() {
		if(onoff == false) {
			...
			$(".swing").css("transform", "rotate(2deg)");
			onoff = !onoff;
			musicStart();
		} else {
			$(this).html("&#xe62e;");
			$(".picbg").css("animation", "");
			$(".swing").css("transform", "rotate(-21deg)");
			onoff = !onoff;
			musicStop();
		}
//		console.log(onoff);
	...
	function musicSpeed() {
		...
		jindu.addEventListener("click", function(e) {
			...
			audio.currentTime = parseInt(speed / 300 * audio.duration);
		})
	}
